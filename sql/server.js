...
function generateUniqueImageName() {
  ...
  const uniqueName = `${timestamp}_${randomSuffix}`; // 构建唯一名称
  return uniqueName;
}
...
app.post('/upload',(req, res) => {
  ...
  const newPath = `./uploads/${folderName}/image_${imgName}.png`;
  ...
  try{
    ...
    fs.readFile(newPath, function (err, data) {
      if (err) {
        ...
        // 将文件内容传输给客户端 
        const imageUrl = `http://localhost:3000/${folderName}/image_${imgName}.png`;  //相对于静态文件目录的URL路径，并将其与服务器的基本URL拼接起来
        res.statusCode = 200;
...
app.post('/demo',(req,res)=>{
  ...
  const sort = req.body
  if(sort.length>0){
    let sql = "select * from tv where sort = ? "
